...
const AddEnrollment = () => {
  ...
  const fetchEnrollmentDetails = async () => {
    try {
      if (id) {
        ...
        const { userId, courseId } = response.data;
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      navigate('/enrollments'); // Navigate back to enrollment management page after submission
    } catch (error) {
            ...
            <form onSubmit={handleFormSubmit}>
              ...
              <div className="input-group">
                <input
                  ...
                  required
                />
                <label className={`${userId.length > 0 ? 'focusLabel' : ''}`}>User ID</label>
              </div>
              <div className="input-group">
                <select
                  value={courseId}
