...
const AddCourse = () => {
  ...
  const fetchCourseDetails = async () => {
    try {
      ...
      setCourseenrolmentid(courseenrolmentid);
    } catch (error) {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      navigate('/courses'); // Navigate back to course management page after submission
    } catch (error) {
            ...
            <form onSubmit={handleFormSubmit}>
              ...
              <div className="input-group">
                <input
                  ...
                  required
                />
                <label className={`${title.length > 0 ? 'focusLabel' : ''}`}>Title</label>
              </div>
              <div className="input-group">
                <textarea
                  value={description}
                  ...
                  required
                />
                <label className={`${description.length > 0 ? 'focusLabel' : ''}`}>Description</label>
              </div>
              ...
              <div className="input-group">
                <textarea
                  ...
                  required
                />
                <label className={`${courseenrolmentid.length > 0 ? 'focusLabel' : ''}`}>CourseEntrolmentID</label>
              </div>
